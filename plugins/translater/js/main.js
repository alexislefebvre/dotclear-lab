...
$(function(){
	...
	/* pack page */
	var pForm=$('#pack-form');
	if ($(pForm).attr('id')!=undefined){
		dotclear.jcTools.formFieldsetToMenu(pForm);
		$(pForm).submit(function(){dotclear.jcTools.waitMessage();return true;});
	}
	/* module page */ // bug as this is not a form
	var mForm=$('#module-form');
	if ($(mForm).attr('id')!=undefined){
		dotclear.jcTools.formFieldsetToMenu(mForm);
		$(mForm).submit(function(){dotclear.jcTools.waitMessage();return true;});
	}
	/* modules page */
	var aHref=$('.wait');
	if ($(aHref).attr('id')!=undefined){
		$(aHref).click(function(){dotclear.jcTools.waitMessage();return true;});
	}
	/* lang page */
	var lForm=$('#lang-form');
	if ($(lForm).attr('id')!=undefined){
		$(lForm).submit(function(){dotclear.jcTools.waitMessage();return true;});
	}
});
