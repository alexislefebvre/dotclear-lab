...
if ($post_id) {
	$post = superAdmin::getPosts(array('post_id'=>$post_id));
	if ($post->isEmpty()) {
		$post_id = null;
	}
...
$page_url = $p_url.'&file=media_item&popup='.$popup.'&post_id='.$post_id;
$media_page_url = $p_url.'&file=media&popup='.$popup.'&post_id='.$post_id;
...
if (is_int($id))
	{
	# load post
	$rs_post_id = $core->con->select('SELECT post_id '.
		'FROM '.$core->prefix.'post_media '.
		'WHERE (post_id = '.$id.')');
	
	$rs = superAdmin::getPosts(array('post_id'=>$rs_post_id->post_id));
...
echo '<h2>'.__('Super Admin').' &rsaquo; <a href="'.html::escapeURL($media_page_url).'">'.__('Media manager').'</a>'.
...
if ($popup)
{
	...
	{
		$media_type = 'image';
		$media_desc = dcGetImageTitle($file,$core->blog->settings->media_img_title_pattern);
	...
	elseif ($file->type == 'audio/mpeg3')
	{
		...
		$public_player = dcMedia::mp3player($file->file_url,$core->blog->getQmarkURL().'pf=player_mp3.swf');
	...
	elseif ($file->type == 'video/x-flv' || $file->type == 'video/mp4' || $file->type == 'video/x-m4v')
	{
		...
		$public_player = dcMedia::flvplayer($file->file_url,$core->blog->getQmarkURL().'pf=player_flv.swf');
		echo form::hidden('public_player',html::escapeHTML($public_player));
		echo '</p>';
	}
	else
