...
if ($post_id) {
	$post = superAdmin::getPosts(array('post_id'=>$post_id,'post_type'=>''));
	if ($post->isEmpty()) {
		$post_id = null;
	}
...
$page_url = $p_url.'&file=media&popup='.$popup.'&post_id='.$post_id;
...
if ($dir && !empty($_GET['remove']))
{
	...
	echo '<h2>'.__('Super Admin').' &rsaquo; '.html::escapeHTML($core->blog->name).' &rsaquo; '.__('Media manager').' &rsaquo; '.__('confirm removal').'</h2>';
	
	echo
	'<form action="'.$p_url.'" method="post">'.
	form::hidden('file','media').
...
echo '<h2>'.__('Super Admin').' &rsaquo; '.html::escapeHTML($core->blog->name).' &rsaquo; <a href="'.html::escapeURL($page_url.'&d=').'">'.__('Media manager').'</a>'.
...
if ($post_id) {
	...
	'<a href="'.$p_url.'&amp;file=post&amp;id='.$post_id.'">'.html::escapeHTML($post_title).'</a>',
...
if ($core_media_writable)
{
	...
	'<form id="media-upload" class="clear" action="'.$p_url.'" method="post" enctype="multipart/form-data">'.
	form::hidden('file','media').
	...
	'<form class="clear" action="'.$p_url.'" method="post">'.
	form::hidden('file','media').
...
echo
'<form id="media-remove-hide" action="'.$p_url.'" method="post"><div class="clear">'.
form::hidden('file','media').
...
{
	global $core, $page_url, $popup, $post_id, $p_url;
	...
	if ($f->d) {
		...
		$link =
		$p_url.'&amp;file=media_item&amp;id='.$f->media_id.'&amp;popup='.$popup.'&amp;post_id='.$post_id;
	}
	...
	if ($post_id && !$f->d) {
		$res .= '<form action="'.$p_url.'" method="post">'.
