...
{
	# load post
	$rs = superAdmin::getPosts(array('post_id' => $_REQUEST['id']));
...
if (!empty($_POST) && !empty($_POST['save']) && $can_edit_post)
{
	...
	if ($post_id)
	{
		try
		{
			...
			http::redirect($p_url.'&file=post&id='.$post_id.'&upd=1');
		}
		catch (Exception $e)
		{
	...
	else
	{
		...
		try
		{
			...
			http::redirect($p_url.'&file=post&id='.$return_id.'&crea=1');
		}
		catch (Exception $e)
		{
			$core->error->add($e->getMessage());
...
if (!empty($_POST['delete']) && $can_delete)
{
	try {
		...
		http::redirect($p_url.'&file=posts');
	} catch (Exception $e) {
		$core->error->add($e->getMessage());
	}
}
...
echo '<h2>'.__('Super Admin').' &rsaquo; '.html::escapeHTML($core->blog->name).' &rsaquo; '.$page_title;
...
if ($can_edit_post)
{
	...
	echo '<form action="'.$p_url.'" method="post" id="entry-form">';
	echo form::hidden('file','post');
	...
	($post_id && $post_format != 'xhtml' ? '<a href="'.$p_url.'&amp;file=post&amp;id='.$post_id.'&amp;xconv=1">'.__('Convert to XHTML').'</a>' : '').
	'</label></p>'.
...
if ($post_id)
{
	...
	if ($has_action) {
		...
		form::hidden('redir',$p_url.'&amp;file=post&amp;id='.$post_id.'&amp;co=1').
		$core->formNonce().
...
if ($post_id)
{
	...
	'<form action="'.$p_url.'" method="post" id="comment-form">'.
	form::hidden('file','comment').
...
function showComments(&$rs,$has_action)
{
	...
	while($rs->fetch())
	{
		...
		'<td class="nowrap"><a href="'.$p_url.'&amp;file=comments'.
			'&amp;ip='.rawurlencode($rs->comment_ip).'">'.$rs->comment_ip.'</a></td>'.
