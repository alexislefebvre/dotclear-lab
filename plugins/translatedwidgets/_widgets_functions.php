...
require dirname(__FILE__).'/_new_widgets_functions.php';
class defaultWidgets
{
	...
	public static function categories(&$w)
	{
		...
		while ($rs->fetch())
		{
			...
			$rs->cat_url.'">'.
			html::escapeHTML(__($rs->cat_title)).'</a>'.
	...
	public static function bestof(&$w)
	{
		...
		// dcTranslations support
		$trans = $core->plugins->moduleExists('dctranslations');
		while ($rs->fetch()) {
			$res .= ' <li><a href="'.$rs->getURL().'">'.html::escapeHTML(($trans?$rs->getTitle():$rs->post_title)).'</a></li> ';
		}
	...
	public static function subscribe(&$w)
	{
		...
		$trans = $core->plugins->moduleExists('dctranslations');
		if ($trans) {
		    $xlang = '/navlang:'.join("~",$core->lang_array);
		    if ($core->url->type == 'lang') {
			$xlang .= '/'.$core->translation_force_lang;
		    }
		} else { $xlang=''; }
		...
		$res .=
		'<li><a type="'.$mime.'" '.
		'href="'.$core->blog->url.$core->url->getBase('feed').$xlang.'/'.$type.'" '.
		...
		if ($core->blog->settings->system->allow_comments || $core->blog->settings->system->allow_trackbacks)
		{
			$res .=
			'<li><a type="'.$mime.'" '.
			'href="'.$core->blog->url.$core->url->getBase('feed').$xlang.'/'.$type.'/comments" '.
	...
	public static function lastposts(&$w)
	{
		...
		while ($rs->fetch()) {
			...
			html::escapeHTML(($trans?$rs->getTitle():$rs->post_title)).'</a></li>';
		}
	...
	public static function lastcomments(&$w)
	{
		...
		while ($rs->fetch())
		{
			...
			// FIXME: get translated title if dctranslations available
