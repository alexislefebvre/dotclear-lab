	...
	public static function loadArray($A,&$widgets)
	{
		...
		foreach ($A as $v)
		{
			if ($widgets->{$v['id']} != null)
			{
				...
				foreach ($v as $sid => $s) {
					if ($sid == 'title') {
						$originalvalue = array_search($s,$GLOBALS['__l10n']);
...
class dcWidget
{
	...
	public function __get($n)
	{
		if (isset($this->settings[$n])) {
            if ($n == 'title')
                return __($this->settings[$n]['value']);
	...
	public function setting($name,$title,$value,$type='text')
	{
		...
		if ($name == 'title') {
			$originalvalue = array_search($value,$GLOBALS['__l10n']);
			if (!($originalvalue===false)) {
				$value=$originalvalue;
			}
		}
	...
	public function formSettings($pr='')
	{
		...
		foreach ($this->settings as $id => $s)
		{
			...
			switch ($s['type'])
			{
				case 'text':
					$res .=
					'<p><label>'.$s['title'].($id=='title'?' '.__('(translated version)').'('.$s['value'].')':'').' '.
					form::field(array($iname),20,255,html::escapeHTML(($id!='title'?$s['value']:__($s['value']))),'maximal').
					'</label></p>';
					break;
				case 'textarea':
					$res .=
