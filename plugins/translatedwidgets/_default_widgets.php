...
$__widgets->subscribe->setting('homeonly',__('Home page only'),1,'check');
...
while ($rs->fetch()) {
	$categories[str_repeat('&nbsp;&nbsp;',$rs->level-1).'&bull; '.html::escapeHTML(__($rs->cat_title))] = $rs->cat_id;
}
...
if ($core->plugins->moduleExists('metadata')) {
    $__widgets->create('mtags',__('Tags').' '.__('(translated version)'),
                       array('translatedWidgets','tagsWidget'));
    $__widgets->mtags->setting('title',__('Title:'),__('Tags'));
    $__widgets->mtags->setting('limit',__('Limit (empty means no limit):'),'20');
    $__widgets->mtags->setting('sortby',__('Order by:'),'meta_id_lower','combo',
                               array(__('Tag name') => 'meta_id_lower', 
                                     __('Entries count') => 'count'));
    $__widgets->mtags->setting('orderby',__('Sort:'),'asc','combo',
                               array(__('Ascending') => 'asc',
                                     __('Descending') => 'desc'));
 }
