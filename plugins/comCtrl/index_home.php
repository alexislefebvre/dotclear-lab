...
# It was borrowed, adapted and inspired by the code from "comments.php" from dotclear
#
...
{
	$combo_action[__('as is')] = 'nothing';
	$combo_action[__('publish')] = 'publish';
	$combo_action[__('unpublish')] = 'unpublish';
	$combo_action[__('mark as pending')] = 'pending';
	$combo_action[__('mark as junk')] = 'junk';
	
	$combo_ranking[__('as is')] = 'nothing';
	$combo_ranking[__('rank -2 (bad)')] = '-2';
	$combo_ranking[__('rank -1')] = '-1';
	$combo_ranking[__('rank  0 (normal)')] = '0';
	$combo_ranking[__('rank +1 ')] = '1';
	$combo_ranking[__('rank +2 (good)')] = '2';
}
...
try {
	...
	$comment_list = new adminCommentCtrlList($core,$comments,$counter->f(0));
} catch (Exception $e) {
...
<html>
<head>
  <title><?php echo __('commentControl'); ?></title>
...
<body>
<?php
echo '<h2>'.html::escapeHTML($core->blog->name).' &rsaquo; '.__('Comments Control').'</h2>';
...
if (!$core->error->flag())
{
	...
	echo
	'<form action="plugin.php?p=comCtrl" method="get" id="filters-form">'.
	...
	if (!$with_spam) {
		...
		if ($spam_count == 1) {
			...
			'<a href="plugin.php?p=comCtrl&amp;status=-2">'.__('Show them.').'</a></p>';
		}
	}
	printf('<p>%s <a href="plugin.php?p=comCtrl&amp;m=analyze">%s</a> %s</p>',__("Use the"),__("Analysis tools"),__("on comments base"));
	# Show comments
	$comment_list->display($page,$nb_per_page,
	'<form action="plugin.php?p=comCtrl&amp;m=process" method="post" id="form-comments">'.
	...
	'<p class="col checkboxes-helpers"></p>'.
		...
		'<p style="text-align: right;">'.__('Selected comments ranking:'). ' '.
			form::combo('ranking',$combo_ranking).'</p>'.
		'<p style="color:red; font-weight:bolder; text-align: right;"><small>'.__('Warning: the following can break your comment status please test this before').'</small></p>'.
		'<p style="text-align: right;">'. __('Selected comments action:'). ' '.
			form::combo('action',$combo_action).
	$core->formNonce().'</p>'.
	'<p style="text-align: right;"><input type="submit" value="'.__('Apply changes').'" /></p>'.
	...
	form::hidden(array('redir'),'plugin.php?p=comCtrl').
