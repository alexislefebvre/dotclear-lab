...
class adminNewsletter
{
	...
	*/
	static public function Adapt($theme = null)
	{
...
class tabsNewsletter
{
	...
	*/
	static public function Settings()
	{
		...
		try
		{
	        ...
	        if (pluginNewsletter::isActive()) $pactive = 'checked';
			else $pactive = '';
			...
			'<legend>'.__('Plugin state').'</legend>'.
				'<form action="plugin.php" method="post" name="state">'.
					...
					'<p>'.
						'<label class="classic">'.form::checkbox('active',1,$pactive).__('active / inactive').'</label>'.
					'</p>'.
					'<p>'.
			...
			'<legend>'.__('Settings').'</legend>'.
				'<form action="plugin.php" method="post" name="settings">'.
					...
					'<p>'.
						'<label for "feditorname">'.__('Editor name:').'</label>'.
						form::field(array('feditorname'),50,255, $feditorname).'<br /><br />'.
						'<label for "feditoremail">'.__('Editor email:').'</label>'.
						form::field(array('feditoremail'),50,255, $feditoremail).
						'<br /><br /><label for "fmode">'.__('Mode:').'</label>'.
					    '<label class="classic">'.form::radio(array('fmode'),'text',$fmode=='text' ? true : false).__('text').'</label><br />'.
					    '<label class="classic">'.form::radio(array('fmode'),'html',$fmode!='text' ? true : false).__('html').'</label>'.
						'<br /><br /><label for "fmaxposts">'.__('Maximum posts:').'</label>'.
						form::field(array('fmaxposts'),50,255, $fmaxposts).					
						'<br /><label class="classic">'.form::checkbox('fautosend',1,$fautosend).__('Automatic send').'</label>'.
						'<br /><label class="classic">'.form::checkbox('fcaptcha',1,$fcaptcha).__('Captcha').'</label>'.
					'</p>'.
					'<p>'.
						'<input type="submit" value="'.__('Save').'" />'.
						'<input type="reset" value="'.__('Cancel').'" />'.
			...
			'<legend>'.__('Theme template adaptation').'</legend>'.
				'<form action="plugin.php" method="post" name="adapt">'.
					...
					'<p></p>'.
					'<p>'.
						'<label for "fthemes">'.__('Theme name:').'</label>'.
			...
			'<fieldset>' .
			'<legend>'.__('Import/Export datas').'</legend>'.
	...
	*/
	static public function About()
	{
        echo
		'<fieldset>' .
		'<legend>'.__('Why this plugin').'</legend>'.
    		__('I wrote this plugin for peoples who don\'t like rss feeds.').
		'</fieldset>'.
		'<fieldset>' .
	...
	*/
	static public function ListBlog()
	{
        ...
        if (!is_object($datas)) echo __('No subscriber for this blog.');
        else
        {
			global $core;
		    try
		...
		try
		{
			...
			if (!empty($_POST['id']))
			{
				...
				else
				{
                    ...
                    if ($subscribed != null) $subscribed = dt::dt2str($settings->date_format, $subscribed).' @'.dt::dt2str($settings->time_format, $subscribed);
                    else $subscribed = __('Never');
                    if ($lastsent != null) $lastsent = dt::dt2str($settings->date_format, $lastsent).' @'.dt::dt2str($settings->time_format, $lastsent);
                    else $lastsent = __('Never');
