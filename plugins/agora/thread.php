...
if (!empty($_REQUEST['id']))
{
	...
	if ($post->isEmpty())
	{
		$core->error->add(__('This thread does not exist.'));
		$can_view_page = false;
	}
	else
	{
		...
		if ($next_rs !== null) {
			$next_link = sprintf($post_link,$next_rs->post_id,
				html::escapeHTML($next_rs->post_title),__('next thread').'&nbsp;&#187;');
		...
		if ($prev_rs !== null) {
			$prev_link = sprintf($post_link,$prev_rs->post_id,
				html::escapeHTML($prev_rs->post_title),'&#171;&nbsp;'.__('previous thread'));
...
if (!empty($_POST) && !empty($_POST['save']) && $can_edit_page)
{
	...
	if ($post_id)
	{
		try
		{
			...
			$core->callBehavior('adminBeforeThreadUpdate',$cur,$post_id);
			
			$core->blog->updPost($post_id,$cur);
			
			# --BEHAVIOR-- adminAfterPageUpdate
			$core->callBehavior('adminAfterThreadUpdate',$cur,$post_id);
	...
	else
	{
		...
		try
		{
			...
			$core->callBehavior('adminBeforeThreadCreate',$cur);
			
			$return_id = $core->blog->addPost($cur);
			
			# --BEHAVIOR-- adminAfterPageCreate
			$core->callBehavior('adminAfterThreadCreate',$cur,$return_id);
...
if (!empty($_POST['delete']) && $can_delete)
{
	try {
		...
		$core->callBehavior('adminBeforeThreadDelete',$post_id);
...
<html>
<head>
  <title><?php echo $page_title.' - '.__('Threads'); ?></title>
  <script type="text/javascript">
  //<![CDATA[
  <?php echo dcPage::jsVar('dotclear.msg.confirm_delete_post',__("Are you sure you want to delete this thread?")); ?>
  //]]>
  </script>
  <?php echo
  dcPage::jsDatePicker().
...
if (!empty($_GET['upd'])) {
		echo '<p class="message">'.__('Thread has been successfully updated.').'</p>';
}
elseif (!empty($_GET['crea'])) {
		echo '<p class="message">'.__('Thread has been successfully created.').'</p>';
}
...
' &rsaquo; <a href="'.$p_url.'">'.__('Threads').'</a> &rsaquo; '.$page_title;

if ($post_id && $post->post_status == 1) {
	echo ' - <a id="post-preview" href="'.$post->getURL().'" class="button">'.__('View thread').'</a>';
} elseif ($post_id) {
	$preview_url =
	$core->blog->url.$core->url->getBase('threadpreview').'/'.
	$core->auth->userID().'/'.
	http::browserUID(DC_MASTER_KEY.$core->auth->userID().$core->auth->getInfo('user_pwd')).
	'/'.$post->post_url;
	echo ' - <a id="post-preview" href="'.$preview_url.'" class="button">'.__('Preview thread').'</a>';
}
...
if ($can_edit_page)
{
	echo '<div class="multi-part" title="'.__('Edit thread').'" id="edit-entry">';
	...
	__('Selected thread').'</label></p>'.
	
	'<p><label>'.__('Thread lang:').
	form::combo('post_lang',$lang_combo,$post_lang,'',5).'</label></p>'.
	
	'<p><label>'.__('Thread password:').
	...
	__('Warning: If you set the URL manually, it may conflict with another thread.').
	'</p>'.
	'</div>';
...
if ($post_id)
{
	$params = array('post_id' => $post_id, 'order' => 'message_dt ASC');
	...
	$has_action = !empty($combo_action) && !$messages->isEmpty();
	
	echo
	'<div id="messages" class="multi-part" title="'.__('Messages').'">';
	
	if ($has_action) {
		echo '<form action="'.$msg_actions_url.'" method="post" id="form-messages">';
	}
	
	
	echo '<h3>'.__('Messages').'</h3>';
	if (!$messages->isEmpty()) {
		showMessages($messages,$has_action);
	} else {
		echo '<p>'.__('No message').'</p>';
	}
	...
	if ($has_action) {
		...
		form::hidden('redir',html::escapeURL($redir_url).'&amp;id='.$post_id.'&amp;mg=1').
		$core->formNonce().
