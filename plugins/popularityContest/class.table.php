...
\b	20080405
\li	headers() invokes part('head')
\li	cells() invokes part('body')

\b	20071122
\li	added __toString() function : can return the table

\b	20071114
\li	added show() function
\li	removed str2tr(), str2th() and str2td() functions, replaced by tag()
\li	fixed bug : closePart() invokes closeRow()

\b	20071112
\li	added headers() and cells() function

\b	20071110
\li	added arg() function

\b	20071110
\li	closePart don't delete anymore the content of the closed part : $this->{$this->part} .= implode('',$this->rows);
\li	openPart invokes closePart, openRow invokes closeRow, this allow to use a shorter syntax with only open* functions
\li	IE bug fixed with headers
\li	changed open* functions to short syntax (openRow() is now row())

\b	20071108
\li	repeatHead added

\b	20071025
\li	removed $class which was duplicate with $extra_html
...
\todo :
	be able to add extra_html to repeatHead()
...
class table
{
	...
	*/
	...
	/**
	create a XHTML tag by change "$str" to "<$tag $extra_html>$str</$str>"
