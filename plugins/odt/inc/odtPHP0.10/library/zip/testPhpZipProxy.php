<?php
/**
 * Checking the PhpZipProxy
 * Zip Extension is required for this proxy
 ...
 */
	...
	$result1 = @$zip->getFromName('content.xml'); //@ in order to hide normal PHP warning here
	...
	$result2 = @$zip->addFromString('content.xml', 'test string'); //@ in order to hide normal PHP warning here
	...
	echo ($result3 === false) ? '[ok] (false)' : '[erreur] (true)'; //@ in order to hide normal PHP warning here
	echo  $sep;
	
	echo 'Test 4: $zip::close() avec une archive non ouverte' . PHP_EOL;
	$result4 = @$zip->close();
	echo ($result4 === false) ? '[ok] (false)' : '[erreur] (true)'; //@ in order to hide normal PHP warning here
	echo  $sep;
