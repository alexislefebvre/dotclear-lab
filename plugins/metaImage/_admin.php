...
$_menu['Plugins']->addItem(__('Meta Image'),'plugin.php?p=metaImage','index.php?pf=metaImage/icon.png',
		preg_match('/plugin.php\?p=metaImage/',$_SERVER['REQUEST_URI']),
		$core->auth->isSuperAdmin());

$core->addBehavior('adminPostFormSidebar',array('metaImageBehaviors','imageField'));
...
class metaImageBehaviors
{
  ...
  public static function imageField(&$post)
  {
    $core = $GLOBALS['core'];
    $bMustHaveImage = $core->blog->settings->must_have_image;
    $min_width  = $core->blog->settings->min_width;
    $min_height = $core->blog->settings->min_height;
    
    # Tentative de récupération de l'image associée
    $objMeta = new dcMeta($GLOBALS['core']);
    $imageName = ($post) ? $objMeta->getMetaStr($post->post_meta,'image') : '';
...
<?php if ($min_width > 0 || $min_height > 0) 
  ...
  public static function setImage(&$cur,&$post_id)
  {
    ...
    if (!empty($_FILES['upfileimage']) && (strlen($_FILES['upfileimage']['name']) > 0))
    {
      ...
      # définintion des largeurs et hauteurs maximales, TODO : à rendre paramétrable
      $min_width  = $core->blog->settings->min_width;
      $min_height = $core->blog->settings->min_height;
      $max_width  = $core->blog->settings->max_width;
      $max_height = $core->blog->settings->max_height;
