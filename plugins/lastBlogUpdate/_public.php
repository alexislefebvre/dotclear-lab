...
function lastBlogUpdateWidgetPublic($w)
{
	...
	if ($w->media_show && $w->media_text) {
		...
		$rs = $core->con->select(
			...
			"WHERE media_path='".$core->con->escape($core->blog->settings->public_path)."' ".
			'ORDER BY media_upddt DESC '.$core->con->limit(1));

		if (!$rs->isEmpty()) {

			$title = ($w->media_title ? 
				'<strong>'.html::escapeHTML($w->media_title).'</strong> ' : '');
			$text = dt::str($w->media_text,strtotime($rs->f('media_upddt')),
				$core->blog->settings->blog_timezone);
	...
	# Nothing to display
	if (!$blog && !$post && !$comment && !$media && !$addons) return;
