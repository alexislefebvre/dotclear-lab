<?php
# -- BEGIN LICENSE BLOCK ----------------------------------
# This file is part of dcAdvancedCleaner, a plugin for Dotclear 2.
# 
# Copyright (c) 2009 JC Denis and contributors
# jcdenis@gdwd.com
# 
# Licensed under the GPL version 2.0 license.
# A copy of this license is available in LICENSE file or at
# http://www.gnu.org/licenses/old-licenses/gpl-2.0.html
# -- END LICENSE BLOCK ------------------------------------

// Language: français 
// Module: dcAdvancedCleaner - 0.1
// Date: 2009-08-28 14:40:44 
// Author: JC Denis, jcdenis@gdwd.com
// Translated with dcTranslater - 0.2.4 

#index.php:148
$GLOBALS['__l10n']['Advanced cleaner'] = 'Nettoyeur avancé';

#inc/class.dc.uninstaller.php:277
$GLOBALS['__l10n']['extra action'] = 'Action additionelle';

#inc/lib.dc.advanced.cleaner.behaviors.php:118
$GLOBALS['__l10n']['There is no module with uninstall features'] = 'Il n\'y a pas de modules avec des fonctions de désinstallation avancée';

#inc/lib.dc.advanced.cleaner.behaviors.php:123
$GLOBALS['__l10n']['List of modules with advanced uninstall features'] = 'Liste des modules avec des fonctions de désinstallation avancée';

#inc/lib.dc.advanced.cleaner.behaviors.php:203
$GLOBALS['__l10n']['Perform selected actions'] = 'Effectuer les actions sélectionnées';

#index.php:175
$GLOBALS['__l10n']['Settings'] = 'Paramètres';

#index.php:30
$GLOBALS['__l10n']['settings'] = 'paramètres';

#index.php:30
$GLOBALS['__l10n']['setting'] = 'paramètre';

#index.php:32
$GLOBALS['__l10n']['plugins'] = 'extensions';

#index.php:32
$GLOBALS['__l10n']['plugin'] = 'extension';

#index.php:33
$GLOBALS['__l10n']['themes'] = 'thèmes';

#index.php:33
$GLOBALS['__l10n']['theme'] = 'thème';

#index.php:36
$GLOBALS['__l10n']['delete table'] = 'effacer la table';

#index.php:37
$GLOBALS['__l10n']['delete cache files'] = 'effacer les fichiers de cache';

#index.php:38
$GLOBALS['__l10n']['delete plugin files'] = 'effacer les fichiers du plugin';

#index.php:39
$GLOBALS['__l10n']['delete theme files'] = 'effacer les fichier du thème';

#index.php:40
$GLOBALS['__l10n']['delete the version number'] = 'effacer le numéro de version';

#index.php:41
$GLOBALS['__l10n']['Uninstall extensions'] = 'Désinstaller des extensions';

#index.php:54
$GLOBALS['__l10n']['delete global settings'] = 'effacer les paramètres globals';

#index.php:55
$GLOBALS['__l10n']['delete blog settings'] = 'effacer les paramètres du blog';

#index.php:56
$GLOBALS['__l10n']['delete all settings'] = 'effacer tous les paramètres';

#index.php:60
$GLOBALS['__l10n']['empty'] = 'vider';

#index.php:74
$GLOBALS['__l10n']['Namespaces registered in dcSettings'] = 'Espaces de noms enregistrés dans dcSettings';

#index.php:75
$GLOBALS['__l10n']['All database tables of Dotclear'] = 'Toutes les tables de la base de donnée Dotclear';

#index.php:76
$GLOBALS['__l10n']['Folders from plugins directories'] = 'Dossiers des répertoires de plugins';

#index.php:77
$GLOBALS['__l10n']['Folders from blog themes directory'] = 'Dossiers du répertoire des thèmes du blog';

#index.php:78
$GLOBALS['__l10n']['Folders from cache directory'] = 'Dossiers du répertoire de cache';

#index.php:79
$GLOBALS['__l10n']['Versions registered in table "version" of Dotclear'] = 'Versions enregistrées dans la table "version" de Dotclear';

#index.php:149
$GLOBALS['__l10n']['Beware: All actions done here are irreversible and are directly applied'] = 'Attention: Toutes les actions menées ici sont irréversibles et sont directement appliquées';

#index.php:150
$GLOBALS['__l10n']['Action successfully done'] = 'Action effectué avec succès';

#index.php:152
$GLOBALS['__l10n']['Records'] = 'Enregistrements';

#index.php:160
$GLOBALS['__l10n']['Folders'] = 'Dossiers';

#index.php:181
$GLOBALS['__l10n']['Activate behaviors'] = 'Activer les behaviors';

#index.php:188
$GLOBALS['__l10n']['Hide Dotclear default properties in actions tabs'] = 'Cacher les propriétés par défaut dans les onglets d\'action';

#index.php:221
$GLOBALS['__l10n']['There is no %s'] = 'Il n\'y a pas de %s';

#index.php:225
$GLOBALS['__l10n']['There are %s %s'] = 'Il y a %s %s';

#index.php:228
$GLOBALS['__l10n']['Objects'] = 'Objets';

#index.php:250
$GLOBALS['__l10n']['Action on selected rows:'] = 'Action sur les lignes selectionnées :';

?>