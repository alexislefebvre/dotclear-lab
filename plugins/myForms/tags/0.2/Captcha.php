...
class MyFormsCaptcha
{
  ...
  public function getImageAsHtml() {
    ...
    return "<img src='data:image/png;base64,".$data."' alt='Captcha' width='100' height='25' style='vertical-align: middle;' />";
  }
  
  public function getReference() {
    return crypt::hmac(MYFORMS_CAPTCHA_MASTER_KEY,$this->key);
  }
  
  public static function isValid($input,$reference) {
    return crypt::hmac(MYFORMS_CAPTCHA_MASTER_KEY,$input) == $reference;
  }
  
  public static function display($refFieldAttr,$inputFieldAttr) {
    $captcha = new MyFormsCaptcha();
    echo $captcha->getImageAsHtml();
    echo '<input type="hidden" '.$refFieldAttr.' value="'.$captcha->getReference().'" />';
