...
$can_view_page = true;
$can_edit_page = $core->auth->check('usage',$core->blog->id);
	...
	else
	{
		...
		if ($next_rs !== null) {
			$next_link = sprintf($post_link,$next_rs->post_id,
				html::escapeHTML($next_rs->post_title),__('next').'&nbsp;&#187;');
		...
		if ($prev_rs !== null) {
			$prev_link = sprintf($post_link,$prev_rs->post_id,
				html::escapeHTML($prev_rs->post_title),'&#171;&nbsp;'.__('previous'));
...
if (!empty($_POST['delete']) && $can_delete)
{
	try {
		...
		http::redirect($redir_url.'&list=all');
	} catch (Exception $e) {
		$core->error->add($e->getMessage());
	}
}
...
<html>
<head>
  <title><?php echo $page_title.' - '.$plural; ?></title>
  <script type="text/javascript">
  //<![CDATA[
  <?php echo dcPage::jsVar('dotclear.msg.confirm_delete_post',__("Are you sure you want to delete this entry?")); ?>
  //]]>
  </script>
  <?php echo
  dcPage::jsDatePicker().
...
' &rsaquo; <a href="'.$p_url.'&amp;type='.$type.'&amp;list=all">'.$plural.'</a> &rsaquo; '.$page_title;

if ($post_id && $post->post_status == 1) {
	echo ' - <a id="post-preview" href="'.$post->getURL().'" class="button">'.__('View').'</a>';
} elseif ($post_id) {
	$preview_url =
	$core->blog->url.$core->url->getBase($type.'preview').'/'.
	$core->auth->userID().'/'.
...
if ($can_edit_page)
{
	echo '<div class="multi-part" title="'.__('Edit').'" id="edit-entry">';
