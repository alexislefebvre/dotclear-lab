...
if (!empty($_POST)) {
	...
	if (!preg_match('/^([a-z]+)-([a-z]+)$/',$new_typename,$m) || !$new_id || !$new_lang) {
...
if ($_GET['edit'] == 1) {
	...
	echo '<p id="id_cat"><label class="required" title="'.__('Required field').'">'.
        ...
        '</label></p>';
	if ($core->blog->settings->kezako->kezako_manylang) {
		echo '<p><label class="required" title="'.__('Required field').'">'.
			__('Description language:').' '.
			form::combo('thing_lang',$lang_combo,$lang?$lang:$core->blog->settings->system->lang,'',5).
            '</label></p>';
		$hidden='';
	} else {
		$hidden=form::hidden(array('thing_lang'),$core->blog->settings->system->lang);
	}
