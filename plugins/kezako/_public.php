...
if ($core->blog->settings->kezako_usecat) {
    $core->tpl->addValue('CategoryDescription',
                         array('kezakoPublic','CategoryDescription'));
 }
$core->tpl->addBlock('TagDescription',
                     array('kezakoPublic','TagDescription'));
...
class kezakoPublic {
    ...
    public static function TagDescription($attr,$content) {
        ...
        $p='$a=kezakoPublic::getDescription($_ctx->meta->meta_id,\'metadata\',$_ctx->meta->meta_type,$core->blog->settings->lang,'.$langarray.');';
        $p.="if (\$a):\n echo \$a;\nelse:\n?>".$content."<?php endif;";
        return '<?php '.$p.' ?>';
    }
