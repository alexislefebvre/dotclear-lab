...
try
{
	...
	if ($core->blog->settings->kezako->kezako_usecat === null) {
		$core->blog->settings->kezako->put('kezako_usecat',0,'boolean','Use kezako for categories',true,true);
		$core->blog->settings->kezako->put('kezako_manylang',0,'boolean','Allow selection of many languages',true,true);
		http::redirect($p_url);
	}
} catch (Exception $e) {
...
if (isset($_POST['kezako_option'])) {
	$setcat = $core->blog->settings->kezako->kezako_usecat;
	$core->blog->settings->kezako->put('kezako_usecat',!empty($_POST['kezako_usecat']),null,null,true,true);
	$core->blog->settings->kezako->put('kezako_manylang',!empty($_POST['kezako_manylang']),null,null,true,true);
...
echo '</head><body><h2>'.html::escapeHTML($core->blog->name).' &rsaquo; '.'Kezako</h2>';
...
echo '<div class="multi-part" id="list" title="'.__('List of descriptions').'">';
...
$strReq='SELECT * FROM '.$core->prefix.'kezako WHERE blog_id = \''.$blog_id.'\' ORDER BY thing_type, thing_subtype, thing_id,thing_lang';
...
echo '<form action="'.$p_url.'" method="post">'.'<p><label class="classic">'.form::checkbox('kezako_usecat',1,$core->blog->settings->kezako->kezako_usecat).' '.__('Describe categories with Kezako').'</label></p>';
echo '<p><label class="classic">'.form::checkbox('kezako_manylang',1,$core->blog->settings->kezako->kezako_manylang).' '.__('Use many languages').'</label></p>';
echo '<p><input type="submit" value="'.__('save').'" />'.form::hidden(array('kezako_option'),'do').$core->formNonce().'</p>'.'</form>';
echo '</div>';
echo '</body></html>';
?>