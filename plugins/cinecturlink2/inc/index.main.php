...
if (!defined('DC_CONTEXT_CINECTURLINK') || DC_CONTEXT_CINECTURLINK != 'main'){return;}

require_once dirname(__FILE__).'/lib.cinecturlink2.list.php';
$cinecturlink2_folder = (string) $core->blog->settings->cinecturlink2->cinecturlink2_folder;
...
if ($action == 'update_categories')
{
	try
	{
		...
		http::redirect($p_url.'&part='.$part.'&tab='.$tab.'&msg='.$action);
	}
	catch (Exception $e)
	{
...
if (!empty($_POST['links']) && $action == 'change_links_category')
{
	try
	{
		...
		http::redirect($p_url.'&part='.$part.'&tab=links&msg='.$action);
	}
	catch (Exception $e)
	{
		$core->error->add(sprintf($messages_errors[$action],$e->getMessage()));
...
if (!empty($_POST['newlink']))
{
	try
	{
		cinecturlink2::test_folder($root,$cinecturlink2_folder);
		...
		if (empty($upd_link_id))
		{
			...
			$C2->addLink($cur);
			http::redirect($p_url.'&part='.$part.'&tab=links&msg=create_link');
		}
		# Update link
		else
		{
			...
			http::redirect($p_url.'&part='.$part.'&tab=links&msg=update_link');
		}
	}
	catch(Exception $e)
	{
...
if ($msg == 'create_category')
{
	echo sprintf($message,__('Category successfully created'));
}
if ($msg == 'delete_categories')
{
	echo sprintf($message,__('Categories successfully deleted'));
}
if ($msg == 'order_categories')
{
	echo sprintf($message,__('Categories successfully reordered'));
}
...
if ($msg == 'create_link')
{
	echo sprintf($message,__('Link successfully created'));
}
if ($msg == 'delete_links')
{
	echo sprintf($message,__('Links successfully deleted'));
}
if ($msg == 'update_links')
{
	echo sprintf($message,__('Links successfully updated'));
}
if ($links->isEmpty())
{
...
else
{
	...
	echo '
	<form action="'.$p_url.'&amp;part=main&amp;tab=links" method="get" id="filters-form">
...
if (!empty($_POST['links']) && $action == 'moveto_change_links_note')
{
	echo '
