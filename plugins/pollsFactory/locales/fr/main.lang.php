<?php
// Language: français 
// Module: pollsFactory - 1.0
// Date: 2010-03-28 14:54:11 
// Translated with dcTranslater - 1.3 

#_admin.php:18
#_admin.php:117
#_admin.php:153
#_admin.php:199
#_admin.php:200
#_admin.php:201
#_admin.php:202
#_admin.php:203
#_admin.php:204
#_admin.php:205
#_admin.php:207
#_admin.php:208
#_admin.php:211
#inc/index.content.php:326
#inc/index.poll.php:264
#inc/index.polls.php:382
#inc/index.post.php:71
#inc/index.result.php:95
#inc/index.result.php:159
#inc/index.result.php:294
#inc/index.setting.php:77
#index.php:37
$GLOBALS['__l10n']['Polls manager'] = 'Gestion de sondages';

#_admin.php:140
#_admin.php:142
#_admin.php:199
$GLOBALS['__l10n']['add polls'] = 'ajouter des sondages';

#_admin.php:154
$GLOBALS['__l10n']['Are you sure you want to remove this poll?'] = 'Étes-vous sur de vouloir retirer ce sondage ?';

#_admin.php:156
$GLOBALS['__l10n']['edit poll'] = 'modifier le sondage';

#_admin.php:157
$GLOBALS['__l10n']['add poll'] = 'ajouter le sondage';

#_admin.php:158
$GLOBALS['__l10n']['remove poll'] = 'retirer le sondage';

#_admin.php:159
$GLOBALS['__l10n']['No more poll'] = 'Pas plus de sondage';

#_admin.php:200
$GLOBALS['__l10n']['remove polls'] = 'retirer des sondages';

#_admin.php:201
#inc/index.polls.php:294
$GLOBALS['__l10n']['open voting'] = 'ouvrir les votes';

#_admin.php:202
#inc/index.polls.php:295
$GLOBALS['__l10n']['close voting'] = 'fermer les votes';

#_admin.php:235
#inc/index.post.php:77
$GLOBALS['__l10n']['There is no polls'] = 'Il n\'y a pas de sondage';

#_admin.php:238
$GLOBALS['__l10n']['add polls to selected entries'] = 'ajouter des sondages aux billets selectionnés';

#_admin.php:268
$GLOBALS['__l10n']['There is no poll for selected entries'] = 'Il n\'y a pas de sondage pour les billets selectionnés';

#_admin.php:271
$GLOBALS['__l10n']['select polls to remove'] = 'selectionner les sondages à retirer';

#_admin.php:298
$GLOBALS['__l10n']['There is no poll that can be published for selected entries'] = 'Il n\'y a pas de sondage pouvant être publié pour les billets selectionnés';

#_admin.php:301
$GLOBALS['__l10n']['select polls to publish'] = 'selectionner les sondages à publier';

#_admin.php:328
$GLOBALS['__l10n']['There is no poll that can be unpublished for selected entries'] = 'Il n\'y a pas de sondage pouvant être mis hors ligne pour les billets selectionnés';

#_admin.php:331
$GLOBALS['__l10n']['select polls to unpublish'] = 'selectionner les sondages à mettre hors ligne';

#_admin.php:358
$GLOBALS['__l10n']['There is no poll that can be marked as pending for selected entries'] = 'Il n\'y a pas de sondage pouvant être marqué en attente pour les billets selectionnés';

#_admin.php:361
$GLOBALS['__l10n']['select polls to mark as pending'] = 'selectionner les sondages à marquer en attente';

#_admin.php:388
$GLOBALS['__l10n']['There is no poll that can be opened for selected entries'] = 'Il n\'y a pas de sondage puvant être ouvert aux votes';

#_admin.php:391
$GLOBALS['__l10n']['select polls to open'] = 'selctionner les sondages à ouvrir aux votes';

#_admin.php:418
$GLOBALS['__l10n']['There is no poll that can be closed for selected entries'] = 'Il n\'y a pas de sondage pouvant être fermer aux votes pour les billets selectionnés';

#_admin.php:421
$GLOBALS['__l10n']['select polls to close'] = 'selectionner les sondages à fermer aux votes';

#_admin.php:447
$GLOBALS['__l10n']['There is no poll on selected entries'] = 'Il n\'y a pas de sondage sur les billets selectionnés';

#_admin.php:450
$GLOBALS['__l10n']['select polls to delete'] = 'selectionner les sondages à effacer';

#_public.php:309
#default-templates/pollsfactory.html:9
#default-templates/pollsfactory.html:10
#default-templates/pollsfactory.html:17
#inc/index.content.php:339
#inc/index.poll.php:284
#inc/index.polls.php:385
#inc/index.post.php:86
#inc/index.result.php:98
#inc/index.result.php:162
#inc/index.result.php:297
#inc/index.setting.php:86
$GLOBALS['__l10n']['Polls'] = 'Sondages';

#_public.php:452
$GLOBALS['__l10n']['Validate'] = 'Valider';

#_public.php:533
$GLOBALS['__l10n']['no vote'] = 'aucun vote';

#_public.php:536
$GLOBALS['__l10n']['one vote'] = 'un vote';

#_public.php:539
#inc/index.result.php:416
$GLOBALS['__l10n']['%s votes'] = '%s votes';

#_public.php:568
$GLOBALS['__l10n']['Some selected responses:'] = 'Quelques réponses selectionnées :';

#_public.php:584
$GLOBALS['__l10n']['Nobody has participated.'] = 'Personne n\'a participé.';

#_public.php:587
$GLOBALS['__l10n']['One people has participated.'] = 'Une personne a participé.';

#_public.php:590
$GLOBALS['__l10n']['%s people participated.'] = '%s personnes ont participé.';

#_public.php:593
$GLOBALS['__l10n']['This poll is closed.'] = 'Ce sondage est fermé.';

#_public.php:608
$GLOBALS['__l10n']['You have already participated to this poll.'] = 'Vous avez déjà participez à ce sondage.';

#_public.php:609
$GLOBALS['__l10n']['Please wait the end of this poll to see results.'] = 'Merci de patienter la fin du sondage pour voir les résultats.';

#_widgets.php:27
#_widgets.php:32
$GLOBALS['__l10n']['Polls list'] = 'Liste des sondages';

#_widgets.php:36
#_widgets.php:96
$GLOBALS['__l10n']['Polls limit:'] = 'Nombre de sondages :';

#_widgets.php:56
$GLOBALS['__l10n']['Vote:'] = 'Vote :';

#_widgets.php:58
$GLOBALS['__l10n']['All'] = 'Tous';

#_widgets.php:59
$GLOBALS['__l10n']['Opened'] = 'Ouvert';

#_widgets.php:60
$GLOBALS['__l10n']['Closed'] = 'Fermé';

#_widgets.php:72
$GLOBALS['__l10n']['Polls of an entry'] = 'Sondages d\'un billet';

#_widgets.php:76
$GLOBALS['__l10n']['Related polls'] = 'Sondages liés';

#_widgets.php:79
#_widgets.php:99
$GLOBALS['__l10n']['Show description'] = 'Afficher la description';

#_widgets.php:82
#_widgets.php:102
#inc/index.setting.php:105
$GLOBALS['__l10n']['Use graphic results'] = 'Utiliser les résultats graphiques';

#_widgets.php:89
#_widgets.php:93
$GLOBALS['__l10n']['Selected polls'] = 'Sondages selectionnés';

#inc/class.pollsfactory.php:121
$GLOBALS['__l10n']['You are not allowed to update polls'] = 'Vous n\'êtes pas autorisé à modifier les sondages';

#inc/class.pollsfactory.php:141
$GLOBALS['__l10n']['You are not allowed to edit this poll'] = 'Vous n\'êtes pas autorisé à modifier ce sondage';

#inc/class.pollsfactory.php:159
$GLOBALS['__l10n']['home page'] = 'la page d\'accueil';

#inc/class.pollsfactory.php:160
$GLOBALS['__l10n']['post pages'] = 'la page d\'un billet';

#inc/class.pollsfactory.php:161
$GLOBALS['__l10n']['static pages'] = 'les pages statiques';

#inc/class.pollsfactory.php:162
$GLOBALS['__l10n']['tags pages'] = 'les pages d\'un tag';

#inc/class.pollsfactory.php:163
$GLOBALS['__l10n']['archives pages'] = 'les pages des archives';

#inc/class.pollsfactory.php:164
$GLOBALS['__l10n']['category pages'] = 'les pages de catégorie';

#inc/class.pollsfactory.php:165
$GLOBALS['__l10n']['entries feed'] = 'le flux des billets';

#inc/class.pollsfactory.php:174
$GLOBALS['__l10n']['multiple choice list'] = 'liste à choix multiples';

#inc/class.pollsfactory.php:175
$GLOBALS['__l10n']['single choice list'] = 'liste à choix unique';

#inc/class.pollsfactory.php:176
$GLOBALS['__l10n']['options box'] = 'liste déroulante';

#inc/class.pollsfactory.php:177
$GLOBALS['__l10n']['text field'] = 'champs de texte';

#inc/class.pollsfactory.php:178
$GLOBALS['__l10n']['text area'] = 'zone de texte';

#inc/class.postoption.php:86
$GLOBALS['__l10n']['You are not allowed to edit post option'] = 'Vous n\'êtes pas autorisé à modifier les options';

#inc/class.postoption.php:105
$GLOBALS['__l10n']['You are not allowed to edit this post option'] = 'Vous n\'êtes pas autorisé à modifier cette option';

#inc/index.content.php:135
$GLOBALS['__l10n']['You must specify query title'] = 'Vous devez spécifier le titre de la question';

#inc/index.content.php:145
$GLOBALS['__l10n']['You must specify query type'] = 'Vous devez spécifier le type de la question';

#inc/index.content.php:267
$GLOBALS['__l10n']['You must specify selection text'] = 'Vous devez spécifier le texte de la selection';

#inc/index.content.php:340
$GLOBALS['__l10n']['Edit content'] = 'Modifier le contenu';

#inc/index.content.php:348
#inc/index.poll.php:289
$GLOBALS['__l10n']['Preview poll'] = 'Prévisualiser le sondage';

#inc/index.content.php:352
#inc/index.poll.php:33
#inc/index.poll.php:290
#inc/index.poll.php:293
#inc/index.polls.php:386
#inc/index.result.php:100
#inc/index.result.php:164
#inc/index.result.php:299
#inc/index.setting.php:88
$GLOBALS['__l10n']['New poll'] = 'Nouveau sondage';

#inc/index.content.php:376
$GLOBALS['__l10n']['complete'] = 'complet';

#inc/index.content.php:377
$GLOBALS['__l10n']['uncomplete'] = 'incomplet';

#inc/index.content.php:385
#inc/index.content.php:431
$GLOBALS['__l10n']['Edit query'] = 'Modifier la question';

#inc/index.content.php:392
#inc/index.result.php:322
$GLOBALS['__l10n']['Queries'] = 'Questions';

#inc/index.content.php:393
$GLOBALS['__l10n']['Action on existing queries:'] = 'Action sur les questions existantes :';

#inc/index.content.php:394
$GLOBALS['__l10n']['reorder queries'] = 'réordonner les questions';

#inc/index.content.php:395
$GLOBALS['__l10n']['delete queries'] = 'effacer les questions';

#inc/index.content.php:397
#inc/index.content.php:397
#inc/index.content.php:416
$GLOBALS['__l10n']['New query'] = 'Nouvelle question';

#inc/index.content.php:400
$GLOBALS['__l10n']['Query'] = 'Question';

#inc/index.content.php:402
#inc/index.content.php:461
$GLOBALS['__l10n']['selections'] = 'selections';

#inc/index.content.php:456
$GLOBALS['__l10n']['Edit this selection'] = 'Editer la selection';

#inc/index.content.php:462
$GLOBALS['__l10n']['Action on existing selections:'] = 'Action sur les selections existantes :';

#inc/index.content.php:463
$GLOBALS['__l10n']['reorder selections'] = 'réordonner les selections';

#inc/index.content.php:464
$GLOBALS['__l10n']['delete selections'] = 'effacer les selections';

#inc/index.content.php:466
#inc/index.content.php:466
#inc/index.content.php:483
$GLOBALS['__l10n']['New selection'] = 'Nouvelle selection';

#inc/index.content.php:469
$GLOBALS['__l10n']['selection'] = 'selection';

#inc/index.content.php:495
$GLOBALS['__l10n']['Edit selection'] = 'Modifier une selection';

#inc/index.poll.php:86
$GLOBALS['__l10n']['This poll does not exist.'] = 'Ce sondage n\'existe pas';

#inc/index.poll.php:113
$GLOBALS['__l10n']['next poll'] = 'sondage suivant';

#inc/index.poll.php:120
$GLOBALS['__l10n']['previous poll'] = 'sondage précèdant';

#inc/index.poll.php:273
#inc/index.poll.php:388
#inc/index.polls.php:40
$GLOBALS['__l10n']['Related posts'] = 'Billets liés';

#inc/index.poll.php:274
$GLOBALS['__l10n']['Are you sure you want to remove this post?'] = 'ëtes vous sur de vouloir retirer ce billet ?';

#inc/index.poll.php:288
#inc/index.polls.php:109
$GLOBALS['__l10n']['Edit poll'] = 'Modifier le sondage';

#inc/index.poll.php:304
$GLOBALS['__l10n']['Don\'t forget to validate your XHTML conversion by saving your poll.'] = 'N\'oubliez pas de valider votre conversion XHTML en sauvant le sondage.';

#inc/index.poll.php:332
$GLOBALS['__l10n']['Poll'] = 'Sondage';

#inc/index.poll.php:337
$GLOBALS['__l10n']['Poll status:'] = 'Status :';

#inc/index.poll.php:351
$GLOBALS['__l10n']['Selected poll'] = 'Sondage selectionné';

#inc/index.poll.php:354
$GLOBALS['__l10n']['Opened poll'] = 'Sondage ouvert';

#inc/index.poll.php:356
$GLOBALS['__l10n']['Poll lang:'] = 'Langue du sondage';

#inc/index.poll.php:365
$GLOBALS['__l10n']['Warning: If you set the URL manually, it may conflict with another poll.'] = 'Attention: Si vous modifer manuellement l\'URL, il peut être en conflit avec un autre sondage.';

#inc/index.poll.php:382
$GLOBALS['__l10n']['edit entry'] = 'modifier le billet';

#inc/index.poll.php:389
$GLOBALS['__l10n']['Uncheck post to remove'] = 'Décocher les billets à retirer';

#inc/index.poll.php:428
$GLOBALS['__l10n']['Edit queries'] = 'Modifier les questions';

#inc/index.poll.php:428
$GLOBALS['__l10n']['Content'] = 'Contenu';

#inc/index.poll.php:431
$GLOBALS['__l10n']['Show results'] = 'Afficher les résultats';

#inc/index.poll.php:431
$GLOBALS['__l10n']['Results'] = 'Résultats';

#inc/index.polls.php:23
$GLOBALS['__l10n']['No poll'] = 'Pas de sondage';

#inc/index.polls.php:41
$GLOBALS['__l10n']['Votes'] = 'Votes';

#inc/index.polls.php:89
#inc/index.polls.php:273
$GLOBALS['__l10n']['closed'] = 'fermé';

#inc/index.polls.php:117
#inc/index.polls.php:118
$GLOBALS['__l10n']['View results of this poll'] = 'Voir les résultats de ce sondage';

#inc/index.polls.php:272
$GLOBALS['__l10n']['opened'] = 'ouvert';

#inc/index.polls.php:403
$GLOBALS['__l10n']['Opened:'] = 'Ouvert :';

#inc/index.polls.php:420
$GLOBALS['__l10n']['Polls per page'] = 'Sondages par page';

#inc/index.polls.php:435
$GLOBALS['__l10n']['Selected polls action:'] = 'Action sur les sondages selectionnés :';

#inc/index.post.php:78
$GLOBALS['__l10n']['go back'] = 'retour';

#inc/index.post.php:82
$GLOBALS['__l10n']['Add polls to posts'] = 'Ajouter des sondages aux billets';

#inc/index.result.php:69
$GLOBALS['__l10n']['Show user'] = 'Voir l\'utilisateur';

#inc/index.result.php:99
$GLOBALS['__l10n']['Poll result'] = 'Résultats du sondage';

#inc/index.result.php:102
$GLOBALS['__l10n']['select poll'] = 'sélectionner un sondage';

#inc/index.result.php:103
$GLOBALS['__l10n']['select poll to view from polls list'] = 'selectionner un sondage à voir depuis la liste des sondages';

#inc/index.result.php:163
#inc/index.result.php:303
$GLOBALS['__l10n']['Results by user'] = 'Résultats par votant';

#inc/index.result.php:168
#inc/index.result.php:298
$GLOBALS['__l10n']['Results by query'] = 'Résultats par question';

#inc/index.result.php:181
$GLOBALS['__l10n']['delete selected users'] = 'supprimer les utilisateurs selectionnés';

#inc/index.result.php:197
#inc/index.result.php:337
$GLOBALS['__l10n']['Responses'] = 'Réponses';

#inc/index.result.php:214
$GLOBALS['__l10n']['User has not answered this query'] = 'L\'utilisateur n\'a pas répondu à cette question';

#inc/index.result.php:223
$GLOBALS['__l10n']['Remove this response from public side'] = 'Enlever cette réponse de la partie publique';

#inc/index.result.php:228
$GLOBALS['__l10n']['Show this response on public side'] = 'Afficher cette réponse sur la partie publique';

#inc/index.result.php:345
#inc/index.result.php:452
$GLOBALS['__l10n']['There is no response for this query.'] = 'Il n\'y a pas de reponse pour cette question.';

#inc/index.result.php:438
$GLOBALS['__l10n']['Show selected responses on public side'] = 'Afficher les réponses selectionnées coté publique';

#inc/index.setting.php:67
$GLOBALS['__l10n']['cookie and IP'] = 'cookie et IP';

#inc/index.setting.php:71
$GLOBALS['__l10n']['when poll is finished'] = 'quand le sondage est terminé';

#inc/index.setting.php:72
$GLOBALS['__l10n']['when people has voted'] = 'quand l\'utilisateur à voté';

#inc/index.setting.php:87
#index.php:35
$GLOBALS['__l10n']['Settings'] = 'Paramètres';

#inc/index.setting.php:92
$GLOBALS['__l10n']['Plugin activation'] = 'Activation de l\'extension';

#inc/index.setting.php:95
$GLOBALS['__l10n']['Enable extension'] = 'Activer l\'extension';

#inc/index.setting.php:98
$GLOBALS['__l10n']['General rules'] = 'Réglages';

#inc/index.setting.php:99
$GLOBALS['__l10n']['User identification:'] = 'Identification de l\'utilisateur :';

#inc/index.setting.php:101
$GLOBALS['__l10n']['Show reponse:'] = 'Afficher les réponses :';

#inc/index.setting.php:109
$GLOBALS['__l10n']['Graphic results settings'] = 'Résultats graphiques';

#inc/index.setting.php:112
$GLOBALS['__l10n']['Title color:'] = 'Couleur du titre :';

#inc/index.setting.php:114
$GLOBALS['__l10n']['Text color:'] = 'Couleur du texte :';

#inc/index.setting.php:118
$GLOBALS['__l10n']['Chart color:'] = 'Couleur du graphique :';

#inc/index.setting.php:120
$GLOBALS['__l10n']['Bar color:'] = 'Couleur de barre :';

#inc/index.setting.php:124
$GLOBALS['__l10n']['Polls display'] = 'Affichage des sondages';

#inc/index.setting.php:125
$GLOBALS['__l10n']['Show on:'] = 'Afficher sur :';

#inc/index.setting.php:135
$GLOBALS['__l10n']['full content'] = 'contenu complet du sondage';

#inc/index.setting.php:135
$GLOBALS['__l10n']['only link to page of poll'] = 'seulement le lien vers la page du sondage';

#inc/index.setting.php:137
$GLOBALS['__l10n']['Place:'] = 'Emplacement :';

#inc/index.setting.php:138
$GLOBALS['__l10n']['after content'] = 'après le contenu';

#inc/index.setting.php:138
$GLOBALS['__l10n']['before content'] = 'avant le contenu';

#inc/lib.pollsfactory.activityreport.php:16
$GLOBALS['__l10n']['Plugin polls factory'] = 'Gestion de sondages';

#inc/lib.pollsfactory.activityreport.php:22
$GLOBALS['__l10n']['new vote'] = 'nouveau vote';

#inc/lib.pollsfactory.activityreport.php:23
$GLOBALS['__l10n']['New vote on poll called "%s" was set'] = 'Un nouveau vote sur le sondage nommé "%s" a été effectué';

#inc/lib.pollsfactory.activityreport.php:31
$GLOBALS['__l10n']['poll creation'] = 'création de sondage';

#inc/lib.pollsfactory.activityreport.php:32
$GLOBALS['__l10n']['Poll called "%s" was created by "%s"'] = 'Un sondage nommé "%s" a été créé par "%s"';

#index.php:30
$GLOBALS['__l10n']['Please wait'] = 'Veuillez patienter';

#index.php:43
$GLOBALS['__l10n']['Configuration successfully saved'] = 'Configuration sauvegardée avec succès';

#index.php:44
$GLOBALS['__l10n']['Poll successfully created'] = 'Sondage crée avec succès';

#index.php:45
$GLOBALS['__l10n']['Poll successfully updated'] = 'Sondage mis à jour avec succès';

#index.php:46
$GLOBALS['__l10n']['Poll successfully deleted'] = 'Sondage effacé avec succès';

#index.php:47
$GLOBALS['__l10n']['Entries successfully removed'] = 'Billets retirer avec succès';

#index.php:48
$GLOBALS['__l10n']['Votes successfully opened'] = 'Votes ouvert avec succès';

#index.php:49
$GLOBALS['__l10n']['Votes successfully closed'] = 'Votes fermés avec succès';

#index.php:50
$GLOBALS['__l10n']['Polls successfully deleted'] = 'Sondages effacés avec succès';

#index.php:51
#index.php:52
#index.php:53
#index.php:54
$GLOBALS['__l10n']['Polls status successfully updated'] = 'Status des sondages mis à jour avec succès';

#index.php:55
$GLOBALS['__l10n']['Polls successfuly mark as selected'] = 'Sondages marqués selectionnés avec succès';

#index.php:56
$GLOBALS['__l10n']['Polls successfuly mark as unselected'] = 'Sondages marqués non selectionnés avec succès';

#index.php:57
$GLOBALS['__l10n']['Queries successfully deleted'] = 'Questions effacées avec succès';

#index.php:58
$GLOBALS['__l10n']['Queries successfully reordered'] = 'Questions réordonnées avec succès';

#index.php:59
$GLOBALS['__l10n']['Query successfully created'] = 'Question créée avec succès';

#index.php:60
$GLOBALS['__l10n']['Query successfully updated'] = 'Question mise à jour avec succès';

#index.php:61
$GLOBALS['__l10n']['New query successfully deleted'] = 'Nouvelle question effacer avec succès';

#index.php:62
$GLOBALS['__l10n']['Options successfully deleted'] = 'Options effacées avec succès';

#index.php:63
$GLOBALS['__l10n']['Options successfully reordered'] = 'Options réordonnées avec succès';

#index.php:64
$GLOBALS['__l10n']['Option successfully created'] = 'Option créée avec succès';

#index.php:65
$GLOBALS['__l10n']['Option successfully updated'] = 'Option mise à jour avec succès';

#index.php:66
$GLOBALS['__l10n']['Responses succesfully selected'] = 'Réponses seletionnées avec succès';

#index.php:67
$GLOBALS['__l10n']['Response successfully selected'] = 'Réponse selectionnée avec succès';

#index.php:68
$GLOBALS['__l10n']['Response successfully unselected'] = 'Réponse déselectionnée avec succès';

#index.php:69
$GLOBALS['__l10n']['Users successfully deleted'] = 'Utilisateurs effacés avec succès';

?>