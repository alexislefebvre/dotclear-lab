...
class DcTaskManager
{
	...
	public function changeObjState($id,$linked) {
		...
		$query.=" AND blog_id='".$core->blog->id."'";
		$rs = $core->con->select($query);
		$state = $rs->fetch()?$rs->f('finished'):0;
		$state = $state==1?0:1;
		$cur = $core->con->openCursor($core->blog->prefix.'TM_object');
		$cur->finished = $state;
		$cur->update("WHERE `order`='$id' AND task_id='$linked' AND blog_id='".$core->blog->id."'");
	}
	
	public function taskPublicVisibility($id) {
		global $core;
		$query = 'SELECT public_show AS nbr FROM '.$core->blog->prefix.'TM_task WHERE id='.$id.' AND blog_id="'.$core->blog->id.'"';
		$rs = $core->con->select($query);
		$state = $rs->fetch()?$rs->f('nbr'):0;
		$state = $state==1?0:1;
		$cur = $core->con->openCursor($core->blog->prefix.'TM_task');
		$cur->public_show = $state;
		$cur->update('WHERE id='.$id.' AND blog_id="'.$core->blog->id.'"');
	}
	
	public function inverseObjOrder($order1,$order2,$taskId) {
		global $core;
		// récupérer l'id des deux objectifs. sachant que leur ordre va être modifié, ce critère n'est pas fiable
		$rs1 = $core->con->select('SELECT id AS id1 FROM '.$core->blog->prefix.'TM_object '
		    .'WHERE `order`='.$order1.' AND blog_id="'.$core->blog->id.'"');
		$rs2 = $core->con->select('SELECT id AS id2 FROM '.$core->blog->prefix.'TM_object '
		    .'WHERE `order`='.$order2.' AND blog_id="'.$core->blog->id.'"');
		$id1 = $rs1->fetch()?$rs1->f('id1'):0;
		$id2 = $rs2->fetch()?$rs2->f('id2'):0;
		// mettre à jour l'ordre
		$cur1 = $core->con->openCursor($core->blog->prefix.'TM_object');
		$cur2 = $core->con->openCursor($core->blog->prefix.'TM_object');
		$cur1->order = $order2;
		$cur2->order = $order1;
		$cur1->update('WHERE id='.$id1.' AND blog_id="'.$core->blog->id.'"');
		$cur2->update('WHERE id='.$id2.' AND blog_id="'.$core->blog->id.'"');
		// verification avec renvoi d'infos, pour le débuggage.
		$rs1b = $core->con->select('SELECT `order` AS ord1 FROM '.$core->blog->prefix.'TM_object '
		    .'WHERE id='.$id1.' AND blog_id="'.$core->blog->id.'"');
		$rs2b = $core->con->select('SELECT `order` AS ord2 FROM '.$core->blog->prefix.'TM_object '
		    .'WHERE id='.$id2.' AND blog_id="'.$core->blog->id.'"');
		$order1b = $rs1b->fetch()?$rs1b->f('ord1'):0;
		$order2b = $rs2b->fetch()?$rs2b->f('ord2'):0;
		return 'task_id = '.$taskId.' :(order = '.$order1.',id = '.$id1.');(order = '.$order2.',id = '.$id2.');'
		."\n".'task_id = '.$taskId.' :(order = '.$order1b.',id = '.$id1.');(order = '.$order2b.',id = '.$id2.');';
	}
	...
	public function modObj($order,$name,$description,$linked) {
		...
		$cur->update('WHERE `order`='.$order.' AND task_id='.$linked." AND blog_id='".$core->blog->id."'");
	}
	...
	public function delTask($id) {
		global $core;
		$q1 = 'DELETE FROM '.$core->blog->prefix.'TM_task WHERE id = '.$id." AND blog_id='".$core->blog->id."'";
		$q2 = 'DELETE FROM '.$core->blog->prefix.'TM_object WHERE task_id='.$id." AND blog_id='".$core->blog->id."'";
		$core->con->execute($q1);
		$core->con->execute($q2);
	}
	...
	public function delObj($id,$linked) {
		global $core;
		$core->con->execute('DELETE FROM '.$core->blog->prefix.'TM_object WHERE `order`='.$id.' AND task_id = '.$linked." AND blog_id='".$core->blog->id."'");
	}
	
	public function showPublicJS() {
	...
	public function showTaskBar() {
		...
		for($i=0;$i<$this->nbrTasks;$i+=1) {
			if ($this->taskPublicView[$i] == 1) {
				...
				<span id="a'.($i+ 1).'" onmouseover="montre(\''.addslashes($this->taskDesc[$i]).'\');" onmouseout="cache();" align="left" alt="\''.addslashes($this->taskDesc[$i]).'\'" ><span id="b'.($i+1).'"><span id="c'.($i+1).'" alt="'.$this->progress[$i].'"></span><span class="taskName">'.$taskNameSplit[$i].'</span></span></span>
				'.$showObj[$i];
			}
		}
		$toReturn.='<noscript>Your Browser may have JavaScript enabled to see this widget</noscript>';
		return $toReturn;
	}

	private function _getTasks() {
		global $core;
		$query = 'SELECT * FROM '.$core->blog->prefix.'TM_task'." WHERE blog_id='".$core->blog->id."'".' ORDER BY id';
	...
	private function _getObj() {
		...
		$query = 'SELECT * FROM '.$core->blog->prefix.'TM_object WHERE blog_id="'.$core->blog->id.'" ORDER BY task_id, `order`';
		$obj = $core->con->select($query);
		$i=''; # incrementeur 1 (index du table des tâches)
		$y=0; # incrementeur 2
	...
	private function _getProgress() {
		...
		for($i=0;$i<$this->nbrTasks;$i++)
		{
			...
			$query2 = 'SELECT COUNT("id") 
				   ...
				   FROM '.$core->blog->prefix.'TM_object 
				   WHERE task_id=' . $this->taskId[$i];
			$query2.=" AND blog_id='".$core->blog->id."'";
