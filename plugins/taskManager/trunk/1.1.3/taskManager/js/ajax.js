	...
	$('.objTable td table').tableDnD({
		...
		onDrop: function(table, row) {
			...
			for (var i=0; i<rows.length; i++) {
				orders[i] = $("#objTable"+tables.id+" #"+rows[i].id+" .order").attr("value");
				rows_order[i] = rows[i].id;
			}
			for (var j=0; j<rows.length; j++) {
				for (var k=(j+1);k<orders.length;k++) {
					if (orders[j]>orders[k]) {
						$("#objTable"+tables.id+" #"+rows_order[j]+" .order").attr("value",orders[k].toString());
						$("#objTable"+tables.id+" #"+rows_order[k]+" .order").attr("value",orders[j].toString());
						params = {
							xd_check: dotclear.nonce,
							order1: orders[j],
							order2: orders[k],
						...
						$.post("plugin.php?p=taskManager",params,function(data){});
						var order_j = orders[j];
						var order_k = orders[k];
						orders[j] = order_k;
						orders[k] = order_j;
					}
				}
...
function publicShow(id,state)
{
	...
	renew="publicShow("+id+","+state_inverse+")";
	$("#taskId"+id+" .taskvisibility").replaceWith('<td onclick="'+renew+'" class="taskvisibility action"><input type="checkbox" '+check+' title="'+task_visibility_text+'"/></td>');
}
...
function del(id,choice,linked)
{
	...
	if (choice==1){
		...
		show=task_deleted;
		typename="#taskId"+id;
		typename2=delete_task;
	}
	if(window.confirm(delete_+typename2))
	{
...
function saveTask(id,add)
{
	...
	if (typeof(add) != "undefined" ) {
		...
		val = $(".add_task").val();
		$(".add_task").replaceWith('<input class="add_task" type="button" onclick="showNewTask('+(id+1)+');" value="'+val+'"/>');
		$(".add_task").show(0);
		val2 = $(".dont_add_task").val();
		$(".dont_add_task").replaceWith('<input class="dont_add_task" type="button" onclick="hideNewTask('+(id+1)+');" value="'+val2+'"/>');
	...
	$.post("plugin.php?p=taskManager",params,function(callback){
		if (typeof(add) != "undefined" ) {
			...
			str = str.match(motif);
			var answer = RegExp.lastMatch;
			$("#taskManager").append(answer);
		}
		$("#answer").html(show).fadeTo(1000,1).fadeTo(1000, 0);
	});
...
function saveObj(order,linked,add)
{
	...
	if (typeof(add) != "undefined" ) {
		...
		$("#taskId"+linked+" .addobjminus").hide(0);
		$("#taskId"+linked+" .addobjplus").show(0);
	...
	$.post("plugin.php?p=taskManager",params,function(callback){
			...
			if (typeof(add) != "undefined" ) {
				...
				str = str.match(motif);
				var answer = RegExp.lastMatch;
				$("#objTable"+linked+" table").append(answer);
			}
	});
